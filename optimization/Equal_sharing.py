...
def Equal_sharing(num_of_vehicles, timesteps,
    ...
    initial_state_ES = np.copy(initial_states)
    ...
    for t in range(np.int(arrival_time[0]) + 1, num_steps - 5):
        ...
        step_initial_SOC = np.copy(initial_state_ES[:vehicle_ending_index])
        ...
        for i in range(vehicle_ending_index):
            if depart_schedule[i] >= t:
                if step_initial_SOC[i] <= terminal_states[i]:
        ...
        for i in range(vehicle_ending_index):
            if depart_schedule[i] >= t:
                if step_initial_SOC[i] <= terminal_states[i]:
                    u_val[i] = np.minimum(shared_power, max_power_u)
        #print(step_initial_SOC)
        #print(u_val)
        initial_state_ES[:vehicle_ending_index] += u_val
        u_mat[:vehicle_ending_index, t] = u_val

    return initial_state_ES, u_mat
...
for current_date in range(1, 100):
    ...
    x, u = Equal_sharing(num_of_vehicles, num_steps,
